...
const Dashboard = () => {
  ...
  const menuItems = [
    { path: "consultas", label: "Consultas", roles: ["medico", "admin"], icon: "bi-people-fill" },
    { path: "laboratorio", label: "Laboratorios", roles: ["laboratorio", "admin"], icon: "bi-droplet-fill" },
    { path: "usuario", label: "Usuarios", roles: ["admin"], icon: "bi-person-badge-fill" },
    { path: "citas", label: "Citas", roles: ["admin"], icon: "bi-person-badge-fill" },
  ];
      ...
      <div className="flex-grow-1 d-flex flex-column">
        ...
        <div className="flex-grow-1 p-4 overflow-auto">
          <Routes>
            ...
            {allowedMenu.some(m => m.path === "consultas") && <Route path="consultas" element={<Consulta />} />}
            {allowedMenu.some(m => m.path === "laboratorio") && <Route path="laboratorio" element={<Laboratorio />} />}
            {allowedMenu.some(m => m.path === "usuario") && <Route path="usuario" element={<Usuario />} />}
            {allowedMenu.some(m => m.path === "citas") && <Route path="citas" element={<Citas />} />}
